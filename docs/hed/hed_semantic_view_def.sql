/*
 * Student Retention semantic model for analyzing at-risk students.
 *
 * Contains multi-factor risk assessments, academic performance metrics, engagement analytics,
 * financial aid information, and recommended interventions. Used for identifying students
 * requiring support and routing retention alerts to appropriate academic staff.
 *
 * Source: VW_HED_RETENTION_RISK_ANALYSIS (dbt view)
 * Data Flow: PostgreSQL → Fivetran → Snowflake → dbt → Semantic View
 */

create or replace semantic view HOL_DATABASE.INDUSTRIES_HIGHER_EDUCATION.HED_AT_RISK_STUDENTS
	tables (
		AT_RISK_STUDENTS as HOL_DATABASE.INDUSTRIES_HIGHER_EDUCATION.VW_HED_RETENTION_RISK_ANALYSIS unique (STUDENT_ID) comment='View of students flagged as at-risk based on multiple retention indicators including GPA, course completion, engagement scores, login activity, and academic standing. Data flows from PostgreSQL through Fivetran to this Snowflake view via dbt transformations.
'
	)
	facts (
